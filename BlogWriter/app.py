...
def get_compiled_graph():
    """
    Creates a singleton Database Connection Pool and compiles the 
    LangGraph agent with Postgres persistence.
    """
    # Define your Postgres URI (You should put this in your .env file)
    ...
    pool = ConnectionPool(
        conninfo=db_uri,
        max_size=10,
        kwargs={"autocommit": True} # LangGraph requires autocommit
    )
    
    # Initialize the checkpointer
    checkpointer = PostgresSaver(pool)
    
    # setup() automatically creates the required tables (checkpoints, checkpoint_writes, etc.)
    # if they don't already exist in your database!
    checkpointer.setup()
    
    # Compile the graph with Postgres memory
    return workflow.compile(checkpointer=checkpointer)

# Initialize the app with Postgres memory
...
def try_stream(graph_app, inputs: Dict[str, Any], config: dict) -> Iterator[Tuple[str, Any]]:
    ...
    """
    try:
        # We use stream_mode="values" to constantly get the updated state
        last_state = None
...
# -----------------------------
# Markdown renderer that supports local & web images
# -----------------------------
...
if run_btn:
    ...
    # Match initial state exactly to agent.py State
    inputs: Dict[str, Any] = {
        "topic": topic.strip(),
        "mode": "",
        ...
        "evidence": [],
        "plan": None,
        "router_decision": {"needs_research": False, "reason": "", "queries": []},
        "sections": [],
        "merged_md": "",
    ...
    for kind, payload in try_stream(app, inputs, config):
        if kind == "error":
            ...
            summary = {
                "needs_research": current_state.get("router_decision", {}).get("needs_research", False) if isinstance(current_state.get("router_decision"), dict) else False,
            ...
            log(f"[{kind}] {json.dumps(summary, default=str)}")
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            # Automatically save the blog to the local directory so the sidebar picks it up
